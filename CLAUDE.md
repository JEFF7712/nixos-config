...
just check         # full validation: agent docs/workflows + local-bin + flake-update + fmt-check + shell/lid-close/wallpaper + qml-lint + quickshell-test + flake check (host evals + laptop-safety) + check-profiles + git diff --check
...
- Overlays live in `overlays/` (imported by `flake.nix`): `local-packages`, `nix-vscode-extensions`, `orca-slicer`.
...
- CI: `check.yml` on push/PR runs on the nas-01 self-hosted runners (`validate` + `profiles` jobs, secret-free by design, tools pinned via `--inputs-from .`); `build-iso.yml` builds the ISO on `hosts/iso/**` or `home/rupan/iso.nix` changes, monthly, or on dispatch, then pushes the closure to Attic (needs the `ATTIC_TOKEN` repo secret). `build-toplevel.yml` builds the laptop toplevel on `**.nix`/`flake.lock` changes (remote builder for `just switch`, capped at 3 jobs so CI cannot OOM nas-01) and pushes it too. The ISO does not publish: GitHub caps release assets at 2 GiB and the ISO does not fit, so it is consumed locally via `just build-iso` + `writeUSB`. Its size check is an advisory tripwire (fails past 6 GiB), not a distribution limit. Runners are declared in homelab-new (`flake/modules/github-runner-nixos.nix`).
