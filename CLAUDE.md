...
- CI: `check.yml` on push/PR runs on the nas-01 self-hosted runners (`validate` + `profiles` jobs, secret-free by design, tools pinned via `--inputs-from .`); `build-iso.yml` builds the ISO on `hosts/iso/**` or `home/rupan/iso.nix` changes, monthly, or on dispatch, then pushes the closure to Attic (needs the `ATTIC_TOKEN` repo secret). The ISO does not publish: GitHub caps release assets at 2 GiB and the ISO does not fit, so it is consumed locally via `just build-iso` + `writeUSB`. Its size check is an advisory tripwire (fails past 6 GiB), not a distribution limit. Runners are declared in homelab-new (`flake/modules/github-runner-nixos.nix`).
