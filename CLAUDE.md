...
- `.gitignore` excludes local tool state (`.claude`, `.codex`, `.superpowers`, `.venv`, `.worktrees`, build artifacts). `.mcp.json` is tracked (`nix-agent` + `mcp-nixos`) and sets `NIX_AGENT_USAGE_LOG`. The activation pin is `programs.nix-agent.flake` on the wrapped binary; `.mcp.json` also sets `NIX_AGENT_FLAKE` but the wrapper `--set` wins.
