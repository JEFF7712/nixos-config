...
toggle_focus() {
  if [ -f "$STATE_FILE" ]; then
    rm "$STATE_FILE"
    noctalia-shell ipc --any-display call wallpaper enableAutomation
    noctalia-shell ipc --any-display call wallpaper random
    ...
    noctalia-shell ipc --any-display call colorScheme set "Peche"
    restore_shell_themes
    notify-send "Focus Mode" "Disabled"
  else
    touch "$STATE_FILE"
    noctalia-shell ipc --any-display call wallpaper disableAutomation
    if command -v jq &> /dev/null; then
        tmp=$(mktemp)
    ...
    noctalia-shell ipc --any-display call colorScheme set "Focus"
    noctalia-shell ipc --any-display call powerProfile toggleNoctaliaPerformance
    if command -v jq &> /dev/null && command -v niri &> /dev/null; then
        OUTPUTS=$(niri msg -j outputs | jq -r 'keys[]')
        for screen in $OUTPUTS; do
            noctalia-shell ipc --any-display call wallpaper set "$BLACK_WALLPAPER" "$screen"
        done
    else
        noctalia-shell ipc --any-display call wallpaper set "$BLACK_WALLPAPER" "eDP-1"
    fi
